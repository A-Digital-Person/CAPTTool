...
while not allow:
    ...
    mainloop()
    try:
         mintarget = int(mintarget) + 90
         maxtarget = int(maxtarget) + 90
    except:
        allow = False
        showerror = True
        showerror2 = False
...
cv2.createTrackbar("Target Min", "OTHER", int(mintarget)-90, 90, lambda x:None)
cv2.createTrackbar("Target Max", "OTHER", int(maxtarget)-90, 90, lambda x:None)
